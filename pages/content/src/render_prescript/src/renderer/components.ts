import { ParamValueElement, StabilizedBlock } from '../core/types';
...
const websiteName = window.location.hostname.toLowerCase().replace(/^www\./i, '').split('.')[0];
/**
...
export const addRawXmlToggle = (blockDiv: HTMLDivElement, rawContent: string): void => {
    ...
    iconSpan.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.4 16.6L4.8 12l4.6-4.6L8 6l-6 6 6 6 1.4-1.4zm5.2 0l4.6-4.6-4.6-4.6L16 6l6 6-6 6-1.4-1.4z" fill="currentColor"/></svg>';
 ...
 */
export const smoothlyUpdateBlockContent = (block: HTMLElement, newContent: string | HTMLElement, isStreaming: boolean = false): void => {
    if (!block) return;
...
export const addExecuteButton = (blockDiv: HTMLDivElement, rawContent: string): void => {
    ...
    const callId = callIdMatch && callIdMatch[1] ? callIdMatch[1] : `call-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
    ...
    const iconSpan = document.createElement('span');
    iconSpan.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8 5v14l11-7z" fill="currentColor"/></svg>';
    iconSpan.style.display = 'inline-flex';
    ...
    executeButton.onclick = () => {
        ...
        try {
            ...
            mcpHandler.callTool(functionName, parameters, (result: any, error: any) => {
                ...
                if (error) {
                    ...
                    const historyPanel = blockDiv.querySelector('.function-history-panel') ||
                        createHistoryPanel(blockDiv, callId, contentSignature);
            ...
            executeButton.disabled = false;
            resultsPanel.style.display = 'block';
            displayResult(resultsPanel, loadingIndicator, false, `Error: ${error instanceof Error ? error.message : String(error)}`);
        }
    };
...
export const extractFunctionParameters = (rawContent: string): Record<string, any> => {
    ...
    while ((match = paramRegex.exec(rawContent)) !== null) {
        ...
        let originalValue = value; // Keep original string for CDATA check
...
const attachResultAsFile = async (
    ...
    if (adapter && typeof adapter.attachFile === 'function' && typeof adapter.supportsFileUpload === 'function' && adapter.supportsFileUpload()) {
        ...
        try {
            ...
            if (!confirmationText) { // Only reset if not successful
                 setTimeout(() => {
                    button.textContent = originalButtonText; // Restore original text
                    button.prepend(iconSpan); // Re-add the icon
                    button.classList.remove('attach-success', 'attach-error');
...
export const displayResult = (
    ...
    if (success) {
        ...
        insertIconSpan.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M16 12l-7-7v4H2v6h7v4l7-7z" fill="currentColor"/></svg>';
        ...
        insertButton.onclick = async () => { // Make handler async
        ...
        attachIconSpan.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="4" y="2" width="16" height="20" rx="2" fill="none" stroke="currentColor" stroke-width="2"/><path d="M8 6h8M8 10h8M8 14h4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>';
        ...
        if (rawResultText.length > MAX_INSERT_LENGTH && adapter && 
            typeof adapter.supportsFileUpload === 'function' && 
            adapter.supportsFileUpload() && WEBSITE_NAME_FOR_MAX_INSERT_LENGTH_CHECK.includes(websiteName)) {
            ...
            document.dispatchEvent(new CustomEvent('mcp:tool-execution-complete', { detail: { result: wrappedResult, skipAutoInsertCheck: false } }));
        }
    } else {
