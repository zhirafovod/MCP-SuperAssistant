...
import { ExecutedFunction, formatExecutionTime, getExecutedFunctionsForCurrentUrl, storeExecutedFunction, getPreviousExecution } from '../mcpexecute/storage';
...
export const updateHistoryPanel = (
  ...
  reExecuteBtn.onclick = () => {
    ...
    let resultsPanel = historyPanel.parentElement?.querySelector(`.function-results-panel[data-call-id="${executionData.callId}"]`) as HTMLDivElement;
...
export const checkAndDisplayFunctionHistory = (
  ...
  if (!exactMatch) {
    const filteredMatches = executedFunctions.filter(func => 
      func.callId === callId && func.contentSignature === contentSignature
    );
  ...
  if (matchingExecutions.length > 0) {
    ...
    console.debug(`Showing only the latest execution from ${matchingExecutions.length} matches for function ${functionName}`);
  }
};
