...
import { addRawXmlToggle, addExecuteButton, setupAutoScroll, smoothlyUpdateBlockContent, extractFunctionParameters } from './components';
...
import { ParamValueElement } from '../core/types';
...
export const executionTracker: ExecutionTracker = {
    ...
    isFunctionExecuted(callId: string, contentSignature: string, functionName?: string): boolean {
        console.debug(`[Debug] isFunctionExecuted called with: callId='${callId}', signature='${contentSignature}', funcName='${functionName || 'undefined'}'`);
        ...
        if (typeof effectiveFunctionName === 'string') { // Check if we have *any* string name
            ...
            console.debug(`[Debug] isFunctionExecuted (Standard Check): Key='${key}', inMemory=${inMemory}, inStorage=${inStorage}`);
        ...
        // Fallback to Legacy Check ONLY if no function name was passed AND none was found in memory
            ...
            console.debug(`[Debug] isFunctionExecuted (Legacy Check): Key='${key}', inMemory=${inMemory}, inStorage=${inStorage}`);
            return inMemory || inStorage;
        }
    },
        ...
        if (contentSignature && !executionTracker.isFunctionExecuted(extractedCallId, contentSignature, functionName)) {
            ...
            const setupAutoExecution = () => {
                ...
                setTimeout(() => {
                    ...
                    if (!currentBlock) {
                        ...
                        for (const block of potentialBlocks) {
                            ...
                            if (match && match[1] === functionDetails.functionName && match[2] === functionDetails.callId) {
                                ...
                                const alreadyExecuted = getPreviousExecution(functionDetails.functionName, functionDetails.callId, functionDetails.contentSignature); 
                                // Removed isBeingProcessed check

                                if (!alreadyExecuted) {
                                    console.debug(`Auto-execute: Found potential replacement block ${replacementBlockId || 'unknown ID'}. Attempting execution.`);
                                    currentBlock = block; // Target the replacement block
                                    break; 
                                } else {
                                     console.debug(`Auto-execute: Replacement block ${replacementBlockId || 'unknown ID'} skipped (already executed).`); // Updated log message
                                }
                            }
                        }
                    }
                    ...
                    if (!currentBlock) {
                        console.debug(`Auto-execute: Block ${blockId} (and suitable replacement) not found in DOM (attempt ${attempts}/${MAX_AUTO_EXECUTE_ATTEMPTS})`);
                        if (attempts < MAX_AUTO_EXECUTE_ATTEMPTS) {
                    ...
                    // Use the imported getPreviousExecution which checks storage
                    const finalCheckExecuted = getPreviousExecution(functionDetails.functionName, functionDetails.callId, functionDetails.contentSignature);
                    if (finalCheckExecuted) {
                         console.debug(`Auto-execute: Function ${functionDetails.functionName} (callId: ${functionDetails.callId}) was found in execution history right before click. Skipping.`);
                    ...
                    if (executeButton) {
                        console.debug(`Auto-execute: Executing function in block ${currentBlock.getAttribute('data-block-id') || blockId} (${functionDetails.functionName}) after DOM stabilization`);
                        executeButton.click();
                        ...
                        console.debug(`Auto-execute: Execute button not found in block ${currentBlock.getAttribute('data-block-id') || blockId} (attempt ${attempts}/${MAX_AUTO_EXECUTE_ATTEMPTS})`);
                        if (attempts < MAX_AUTO_EXECUTE_ATTEMPTS) {
                            setTimeout(setupAutoExecution, 500); // Retry
                        } else {
...
export const createOrUpdateParamElement = (
    ...
    if (isStreaming) {
        ...
        const scrollToBottom = () => {
            if (paramValueElement.scrollHeight > paramValueElement.clientHeight && !(paramValueElement as any)._userHasScrolled) {
