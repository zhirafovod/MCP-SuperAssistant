...
import { monitorNode, streamingObservers, updateQueue, streamingLastUpdated } from './streamObserver';
import { StabilizedBlock } from '../core/types';
import { streamingContentLengths } from '../parser/index';
import { startProgressiveUpdates } from './streamObserver';
import { preExistingIncompleteBlocks, startStalledStreamDetection, stopStalledStreamDetection } from './stalledStreamHandler';
...
export const processUpdateQueue = (): void => {
    ...
    try {
        ...
        const hasLargeStreaming = Array.from(streamingContentLengths.values())
            .some(length => length > CONFIG.largeContentThreshold);
            ...
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                for (const node of Array.from(mutation.addedNodes)) {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        const isTargetElement = CONFIG.targetSelectors.some(selector => 
                            element.matches(selector));
                        
                        const hasTargetElements = element.querySelectorAll(
                            CONFIG.targetSelectors.join(',')
                        ).length > 0;
                        ...
                        const hasStreamingContainers = element.querySelectorAll(
                            CONFIG.streamingContainerSelectors.join(',')
                        ).length > 0;
                        ...
                        if (isTargetElement || hasTargetElements || isStreamingContainer || hasStreamingContainers || potentialFunctionCall) {
                            shouldProcess = true;
                            break;
                        }
