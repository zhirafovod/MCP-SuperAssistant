(() => {
      ...
      if (lines.length > 0) {
        ...
        for (const lang of CONFIG.knownLanguages) {
          if (firstLine.startsWith(lang)) {
            ...
            return {
              tag: lang,
              content: firstLine.substring(lang.length) + 
                      (lines.length > 1 ? '\n' + lines.slice(1).join('\n') : '')
            };
          }
        }
    ...
    const renderFunctionCall = (block) => {
      ...
      if (!block || 
          !functionInfo.hasFunctionCalls || 
          block.closest('.function-block')) {
        return false;
      }
      
      // Check if this is a block we're already tracking for progressive updates
      const blockId = block.getAttribute('data-block-id') || `block-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      if (invokeMatch || functionInfo.hasParameters) { // Process if invoke OR parameters detected
              ...
              if (param.isNew || paramStillStreaming) {
                  ...
                  if (isStreamingContent) {
                      ...
                      if (!preElement) {
                           ...
                           let currentRenderedLength = parseInt(existingParamValue.getAttribute('data-rendered-length') || '0', 10);
                  ...
                  } else if (param.isNew) { // Only update non-content if it's new
      ...
      if (isNewRender) {
         ...
         if (nextSibling && nextSibling.classList && nextSibling.classList.contains('function-block') &&
            ...
            paramValueDiv._autoScrollObserver = new MutationObserver((mutations) => {
                ...
                if (!paramValueDiv._scrollTimeout) {
                    paramValueDiv._scrollTimeout = setTimeout(() => {
                        if (paramValueDiv.hasAttribute('data-streaming')) { // Only scroll if still streaming
                           autoScrollToBottom();
                        }
                        paramValueDiv._scrollTimeout = null;
    ...
    const processFunctionCalls = () => {
      ...
      try {
        ...
        const newFunctionCallBlocks = allPreBlocks.filter(block => {
          const functionInfo = containsFunctionCalls(block);
          return functionInfo.hasFunctionCalls && 
        ...
        const existingBlocks = allPreBlocks.filter(block => {
          return processedElements.has(block) && 
        ...
        if (count > 0 && CONFIG.debug) {
          console.log(`Processed ${count} function call blocks (${newFunctionCallBlocks.length} new, ${existingBlocks.length} updated)`);
        }
    ...
    const checkForUnprocessedFunctionCalls = () => {
      ...
      const unprocessedBlocks = Array.from(document.querySelectorAll('pre'))
        .filter(block => 
          containsFunctionCalls(block) && 
          !processedElements.has(block) &&
          !block.closest('.function-block')
        );
        ...
        if (mutation.type === 'childList') {
            ...
            for (const node of mutation.addedNodes) {
              ...
              if (childPreNodes.length > 0) {
                for (const preNode of childPreNodes) {
                  if (containsFunctionCalls(preNode) && 
              ...
              if (!shouldProcess) {
                const isStreamingContainer = CONFIG.streamingContainerSelectors.some(selector => 
          ...
          if (mutation.removedNodes.length > 0) {
            for (const node of mutation.removedNodes) {
              ...
              if (node.classList && node.classList.contains('function-block') && 
    ...
    const processUpdateQueue = () => {
      ...
      try {
        ...
        const hasLargeStreaming = Array.from(streamingContentLengths.keys())
          .some(key => streamingContentLengths.get(key) > CONFIG.largeContentThreshold);
    ...
    const checkStreamingUpdates = () => {
      ...
      preBlocks.forEach(block => {
        ...
        const isPotentiallyVisible = document.body.contains(block) || (renderedBlock && document.body.contains(renderedBlock));
    ...
    const startProgressiveUpdates = () => {
      ...
      progressiveUpdateTimer = setInterval(() => {
        ...
        preBlocks.forEach(block => {
          ...
          const hasLargeContent = Array.from(streamingContentLengths.keys())
            .some(key => key.startsWith(`${blockId}-`) && streamingContentLengths.get(key) > CONFIG.largeContentThreshold);
